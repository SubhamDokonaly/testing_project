...
module.exports = function (app, io) {
    ...
    validator.registerUser =
        [
            ...
            check('data.*.password').isLength({ min: 8, max: 16 }).withMessage('Password should be min 6 char and max 16 char'),
            (req, res, next) => {
                const errors = validationResult(req).array()
                if (errors.length > 0) {
    ...
    validator.checkGst =
        [
            check('data').notEmpty().withMessage('Data cannot be empty'),
            check('data.*.gstNumber').notEmpty().withMessage('GST cannot be empty'),
            (req, res, next) => {
                const errors = validationResult(req).array()
                if (errors.length > 0) {
